...
const I18N_DICT = {
  ...
  'sec2.ew_cap':         { zh: 'K=4，等权加权。CA1（深蓝实线）期末净值约700万元（初始100万），最大回撤仅-20.9%。非线性条件模型（CA0-CA3）全面领先传统线性模型（FF/PCA）。沪深300（黑虚线）几乎未涨。等权结果确认CA1的优势并非由大市值股票拉动。',
  ...
  'sec2.ew_perf_title':  { zh: '等权表现汇总（2020.01-2024.12，扣除交易成本）', en: 'Equal-Weighted Performance Summary (2020.01-2024.12, After Transaction Costs)' },
  'sec2.vw_title':       { zh: '市值加权结果', en: 'Value-Weighted Results' },
  'sec2.vw_cap':         { zh: 'K=4，市值加权。CA1期末净值约400万元（初始100万），最大回撤-27.3%。非线性条件模型（CA0-CA3）全面领先传统线性模型（FF/PCA）。沪深300（黑虚线）几乎未涨。',
  ...
  'sec2.table_note':     { zh: '注：Top-Decile仅做多组合，月度调仓。扣除A股实际交易成本（印花税0.05%卖方、佣金0.025%双向、过户费0.002%双向）。加粗=列最优，绿=正收益，红=负收益。FF限K≤4。',
  ...
  // ---- Section 5: Methods (restructured) ----
  'sec5.title':           { zh: '研究方法', en: 'Research Methods' },
  'sec5.data_title':      { zh: '数据来源', en: 'Data Sources' },
  ...
  'sec5.factor_title':    { zh: '特征因子', en: 'Characteristic Factors' },
  ...
  'sec5.robust_title':    { zh: '稳健性检验与异质性', en: 'Robustness & Heterogeneity' },
  'sec5.robust_intro':    { zh: '为确保结论可靠，进行了三项稳健性检验：', en: 'Three robustness checks were conducted to ensure reliability:' },
  'sec5.robust_1':        { zh: '<strong>更换缺失值填充阈值（10/14/18）：</strong>模型相对排序保持稳定，CA0和CA1始终最优。', en: '<strong>Alternative missing-value thresholds (10/14/18):</strong> Model rankings remain stable; CA0 and CA1 consistently optimal.' },
  'sec5.robust_2':        { zh: '<strong>剔除市值最小30%股票：</strong>按 Liu et al. (2019) 去除"壳价值"污染后，CA模型优势依然成立。', en: '<strong>Remove smallest 30% by market cap:</strong> Following Liu et al. (2019) to eliminate shell-value contamination; CA advantage persists.' },
  'sec5.robust_3':        { zh: '<strong>缩短训练期（9年-5年）：</strong>深层网络(CA2/CA3)因过拟合下降更明显，CA0/CA1样本效率最优。', en: '<strong>Shortened training (9yr-5yr):</strong> Deeper networks (CA2/CA3) degrade more due to overfitting; CA0/CA1 show best sample efficiency.' },
  'sec5.hetero_title':    { zh: '<strong>异质性分析：</strong>', en: '<strong>Heterogeneity Analysis:</strong>' },
  'sec5.hetero_text':     { zh: '在不同行业板块（制造业、金融、科技、消费等）分别检验了CA1策略的表现。结果显示模型在各行业间均保持正向超额收益，表明CA提取的因子具有跨行业的定价能力，而非仅由个别行业驱动。行业层面的因子暴露差异反映了不同行业中公司特征定价的异质性。',
  ...
  'sec5.pdf_note':        { zh: '完整的稳健性检验细节、异质性分析结果及定价误差诊断，请参见完整论文 PDF。',
